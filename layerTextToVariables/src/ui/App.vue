...
const registerWithModes = () => {
  const finalPrefix = selectedPrefix.value || newPrefix.value
...
const translateTexts = async () => {
  ...
  for (const pair of extractedPairs.value) {
    ...
    try {
      ...
      translatedTexts.value[pair.header] = String(data.responseData.translatedText).replace(/\.$/, '').trim()
    } catch (_e) {
      translatedTexts.value[pair.header] = pair.header
    }
  }
...
onMounted(() => {
  window.onmessage = (event: MessageEvent) => {
    const msg = (event.data as { pluginMessage?: { type: string; pairs?: Pair[]; data?: Collection[] } }).pluginMessage
    if (!msg) return
    if (msg.type === 'text-extracted' && msg.pairs) {
      extractedPairs.value = msg.pairs
      msg.pairs.forEach(p => { if (!translatedTexts.value[p.header]) translatedTexts.value[p.header] = '' })
      translateTexts()
    } else if (msg.type === 'collections-list' && msg.data) {
...
.btn-next { background: #18A0FB; color: white; border: none; padding: 10px; border-radius: 6px; width: 100%; cursor: pointer; font-size: 12px; font-weight: 600; }
.btn-next:disabled { background: #ccc; cursor: not-allowed; }
.btn-variable { background: #18A0FB; color: white; border: none; padding: 12px; border-radius: 6px; width: 100%; cursor: pointer; font-weight: 600; font-size: 13px; margin-top: 12px; }
