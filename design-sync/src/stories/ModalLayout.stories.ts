  ...
  render: () => ({
    template: `
      <div class="flex gap-6">
        ...
        `, '<UiButton variant="destructive">삭제</UiButton>', 'w-90')}
      </div>
    `,
  }),
}
...
export const UserModal: Story = {
  ...
  render: () => ({
    template: `
      <div class="bg-background rounded-md shadow-bottom flex flex-col min-w-60">
        <div class="flex justify-between items-center px-4 h-14 shrink-0">
        ...
        <div class="overflow-y-auto max-h-[calc(100vh-12rem)] px-6 py-2">
          <div class="px-1 pb-6 grid gap-4">
            ...
            <section class="grid gap-4">
              <dl class="grid gap-1"><dt class="font-bold">소속</dt><dd class="text-secondary">JKND</dd></dl>
              <dl class="grid gap-1"><dt class="font-bold">부서</dt><dd class="text-secondary">thisisneverthat</dd></dl>
              <dl class="grid gap-1"><dt class="font-bold">이메일</dt><dd class="text-secondary">user@gmail.com</dd></dl>
              <dl class="grid gap-1"><dt class="font-bold">연락처</dt><dd class="text-secondary">010-0000-0000</dd></dl>
            </section>
          </div>
        </div>
      </div>
...
export const SettingModal: Story = {
  name: 'Modal — 설정 모달',
  argTypes: {
    groupHeader: { control: 'boolean', description: '그룹 헤더 테이블 여부 (true: 고정 기능 없음)' },
  },
  args: { groupHeader: true },
  render: (args) => ({
    setup() { return { args } },
    template: `
      <div class="bg-background rounded-md shadow-bottom flex flex-col" :class="args.groupHeader ? 'min-w-90' : 'w-180'">
        <div class="flex justify-between items-center px-4 h-14 shrink-0">
        ...
        <div class="overflow-y-auto max-h-[calc(100vh-12rem)] px-6 py-2">
          <div class="flex gap-8 pb-4">
            ...
            <div v-if="!args.groupHeader" class="flex flex-col gap-2 w-1/2">
            ...
            <!-- 일반 항목 -->
            <div :class="['flex flex-col gap-2', args.groupHeader ? 'w-full' : 'w-1/2']">
              <p v-if="args.groupHeader" class="text-muted caption flex items-center gap-1 justify-end mb-4">
...
export const DataAddModal: Story = {
  ...
  render: (args) => ({
    ...
    setup() { return { args } },
    template: `
      <div :class="['bg-background rounded-md shadow-bottom flex flex-col w-[860px]', args.splitType === 'vertical' ? 'h-[620px]' : '']">
        <div class="flex justify-between items-center px-4 h-14 shrink-0">
      ...
      <div class="bg-background rounded-md shadow-bottom flex flex-col w-120 min-h-120">
        ...
        <div class="overflow-y-auto max-h-[calc(100vh-12rem)] px-6 py-2">
          ...
          <div class="my-4">
            ...
            <div class="py-3 flex items-center justify-center gap-1">
              <UiButton variant="ghost" size="icon-sm"><LucideChevronLeft /></UiButton>
              <UiButton variant="ghost" size="sm" class="text-primary font-bold">1</UiButton>
              <UiButton variant="ghost" size="sm">2</UiButton>
              <UiButton variant="ghost" size="sm">3</UiButton>
              <UiButton variant="ghost" size="icon-sm"><LucideChevronRight /></UiButton>
            </div>
          </div>
        </div>
      </div>
